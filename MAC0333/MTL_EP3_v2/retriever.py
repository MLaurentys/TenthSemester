...
class Retriever:
    def __init__(
        ...
        indMap = self._RemoveFromIndex(toRemove)  # Changes files and encodings
    ...
    def HandleSelect(self, tokens):
        ...
        for token in tokens:
            ...
            print(f" {len(incidence):{4}} | {token:{13}} | " f"{incidence}")
        ...
        self._PrintSelect(tokens, commonFiles, results)
    ...
    def _SortMethod0(self, tokens, commonFiles):
        results = {file:'-' for file in commonFiles}
        return commonFiles, results
    ...
    def _SortMethod3(self, tokens, commonFiles):
        def CompareValue(docID):
            ...
            results[docID] = [smallest[0] + 1, smallest[1] + 1]
    ...
    def _SortMethod4(self, tokens, commonFiles):
        def CompareValue(docID):
            ...
            smallest = util.GetSmallestIntervalSimplified(positions)
            results[docID] = [smallest[0] + 1, smallest[1] + 1]
            return smallest[1] - smallest[0]
    ...
    def _PrintFile(self, docID, interval):
        inter = [interval[0] - 1, interval[1] - 1]
        ...
        inter[1] = min(inter[1] + 3, len(generatedWords) - 1)
        return " ".join(generatedWords[inter[0] : inter[1] + 1])

    def _PrintSelect(self, tokens, commonFiles, results):
        print(
            f"Existem {len(commonFiles)} documentos com os {len(tokens)} "
            f"termos:"
        )
        showed = 0
        if self.verb:
