...
def GetSmallestIntervalSimplified(lists):
    minDist = maxsize
    minInterval = [0, maxsize]
    ...
    while True:
        ...
        for i in range(len(lists)):
            if indices[i] == sizes[i]:
                return minInterval
            if lists[i][indices[i]] < minEl:
                minEl = lists[i][indices[i]]
                minInd = i
            if lists[i][indices[i]] > maxEl:
                maxEl = lists[i][indices[i]]

        if maxEl - minEl < minInterval[1] - minInterval[0]:
            minInterval[1] = maxEl
            minInterval[0] = minEl
